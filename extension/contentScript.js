...
(() => {
  ...
  function classifyPromptComplexity(text) {
    ...
    for (var i = 0; i < complexPatterns.length; i++) {
      if (lower.indexOf(complexPatterns[i]) !== -1) return 'high reasoning';
    }
    for (var j = 0; j < mediumPatterns.length; j++) {
      if (lower.indexOf(mediumPatterns[j]) !== -1) return 'medium reasoning';
    }

    var questionCount = (lower.match(/\?/g) || []).length;
    if (questionCount >= 2) return 'high reasoning';

    if (len > 60) return 'high reasoning';
    if (len > 30)  return 'medium reasoning';
